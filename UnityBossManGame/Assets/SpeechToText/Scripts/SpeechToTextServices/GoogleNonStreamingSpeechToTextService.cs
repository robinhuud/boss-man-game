...
namespace UnitySpeechToText.Services
{
    ...
    public class GoogleNonStreamingSpeechToTextService : NonStreamingSpeechToTextService
    {
        ...
        protected override IEnumerator TranslateRecordingToText()
        {
            ...
            UnityWebRequest request = new UnityWebRequest(Constants.GoogleNonStreamingSpeechToTextURL +
    "?" + Constants.GoogleAPIKeyParameterName + "=" + m_APIKey, UnityWebRequest.kHttpVerbPOST);
            UploadHandlerRaw uH = new UploadHandlerRaw(System.Text.Encoding.UTF8.GetBytes(requestJSON.ToString()));
            ...
            var responseJSON = new JSONObject(request.downloadHandler.text, int.MaxValue);
